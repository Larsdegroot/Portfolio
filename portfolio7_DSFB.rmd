...
dbWriteTable(con, "dengue data", dengue, overwrite = T)
dbWriteTable(con, "flu data", flu, overwrite = T)
dbWriteTable(con, "gapminder data", gapminder, overwrite = T)
...
```{sql connection= con}
SELECT *  FROM "dengue data";
```
```{sql connection= con}
SELECT *  FROM "flu data";
```

```{sql connection= con}
SELECT *  FROM "gapminder data";
```
...
flu <- dbReadTable(conn = con, "flu data")
dengue <- dbReadTable(conn = con, "dengue data")
gapminder <- dbReadTable(conn = con, "gapminder data")
```
